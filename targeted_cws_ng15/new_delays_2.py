...
def cw_delay_new(toas, pos, pdist, name,
                 ...
                 dropout_psr=None, k_drop=None, k_threshold=0):
    """
    ...
    :param dropout_psr:
        Name of pulsar to compute dropout parameter. Default None.
    :param k_drop:
        dropout parameter. Input either a fixed value or uniform distribution
    :param k_threshold:
        threshold for the dropout parameter. Default 0 (so negative drops out, positive keeps in).
    :return: Vector of induced residuals
    """
    # return no delay for this pulsar in a dropout analysis
    if dropout_psr is not None:
        if name == dropout_psr and k_drop < k_threshold:
    ...
    At = -0.5*np.sin(2*phase)*(3+np.cos(2*inc)) # equivalent to -np.sin(2*phase)*(1+np.cos(inc)**2)
    Bt = 2*np.cos(2*phase)*np.cos(inc)
    At_p = -0.5*np.sin(2*phase_p)*(3+np.cos(2*inc)) # equivalent to -np.sin(2*phase_p)*(1+np.cos(inc)**2)

...
from enterprise_extensions.models import (dm_noise_block,
...
def cw_model_2(psrs, priors, noisedict_path, psr_distance_path=None,
               ...
               log10_mc_prior='normal', log10_fgw_prior='normal', normal_pdist_priors=False,
               vary_fgw=False, all_sky=False, fixedpoint=False, nogwb=False,
               free_p_phases=True, dropout=None, inc_ecorr=True, psrTerm=True,
               debug_cw=False):
    
    """
    psrs: list of pulsar objects
    priors: dictionary or pandas series with target's info and at least the following keys:
        [RA, DEC, log10_dL, log10_freq]
        Also need [log10_Mc, log10_Mc_sigma] to run with an astro-informed prior
    ...
    free_p_phases: Set True to sample an additional pulsar phase factor for
        ...
        distance
    dropout: psrname to run CW dropout analysis on. Default None.
    psrTerm: Toggle True to include the pulsar term. False for Earth term only.
    debug_cw: Toggle True to only add a CW in the model and add print statements
    """
    ...
    if not nogwb:
        s += common_red_noise_block(psd=gw_psd, prior='log-uniform',
                                    ...
                                    gamma_val=gamma_val, orf=orf, delta_val=0,
    ...
    if vary_fgw:
        ...
        if log10_fgw_prior == 'uniform':
            print('Varying log10_fgw w/ log-uniform prior')
            log10_fgw = parameter.Uniform(np.log10(1/Tspan), -7)('log10_fgw')
        elif log10_fgw_prior == 'normal':
            # this is calibrated to put 8*binary orbital period at 3 sigma
            log10_fgw_sigma = (-np.log10(10**priors['log10_freq']/16) + priors['log10_freq'])/3
            print(f"Targeted log10_fgw prior: mu={priors['log10_freq']}, sigma={log10_fgw_sigma}")
            # using TruncNormal, since we shouldn't go below dataset timespan or above 100 nHz 
            log10_fgw = parameter.TruncNormal(mu=priors['log10_freq'], sigma=log10_fgw_sigma,
                                              pmin=np.log10(1/Tspan), pmax=-7)('log10_fgw')
    else:
        print(f"log10_fgw is constant log10_fgw = {priors['log10_freq']}")
    ...
    elif log10_mc_prior == 'uniform' or log10_mc_prior == 'uniform_low':
        log10_mc = parameter.Uniform(7,11)('log10_mc')
    ...
    if dropout is not None:
        k_drop = parameter.Uniform(-1,1)(f'{dropout}_k_drop')
    else:
        k_drop = None
    
    # Everything else depends on pulsar distance
    print('No scaling/shifting of pulsar distances')
    models = []
    for p in psrs:
        ...
        wf = nd.cw_delay_new(cos_gwtheta=cos_gwtheta, gwphi=gwphi,
                             ...
                             check=False, tref=tref, scale_shift_pdists=False,
                             dropout_psr=dropout, k_drop=k_drop)
        s2 += CWSignal(wf, ecc=False, psrTerm=psrTerm, name = 'cw')
        
        if debug_cw:
            #s2 = CWSignal(wf, ecc=False, psrTerm=True, name = 'cw') # no gwb or noise
            print({'cos_gwtheta':cos_gwtheta, 'gwphi':gwphi, 'cos_inc':cos_inc, 'log10_mc':log10_mc,
                   'log10_fgw':log10_fgw, 'log10_dist':log10_dL, 'phase0':phase0,
                   'psi':psi, 'psrTerm':psrTerm, 'p_dist':p_dist, 'p_phase':p_phase,
                   'evolve':True, 'check':False, 'tref':tref, 'scale_shift_pdists':False,
                   'dropout_psr':dropout, 'k_drop':k_drop})
...
def gwb_only_model(psrs, noisedict_path, orf=None,
                   ...
                   fixedpoint=False, inc_ecorr=True):
    ...
    s += common_red_noise_block(psd=gw_psd, prior='log-uniform', Tspan=Tspan,
                                ...
                                orf=orf, name=crn_name, delta_val=0,
...
def custom_noise_block(psrname, params, fixedpoint=False,
    ...
    if len(expdip_params) > 0:
        ...
        idxs = [parameter.Constant(params[f'{psrname}_exp{i}_idx']) for i in range(1,3)]
        basenames = ['exp1', 'exp2']

...
try:
    ...
    print('missing custom packages: ipta_gwb_analysis, DR3_noise_modeling')
    print('a few functions may require some tweaking')

def get_prior_distr(core, param, Npoints=1000, return_prior_obj=False):
    ...
    if prior_type == 'Uniform':
        ...
        x = np.linspace(pmin, pmax, 300)
        prior = parameter.NormalPrior
        y = prior(x, mu, sigma)
        #y = prior_dist.pdf(x)
    elif prior_type == 'TruncNormal':
        mu = float(pline[pline.index('mu')+3:pline.index(', sigma')])
        sigma = float(pline[pline.index('sigma')+6:pline.index(', pmin')])
        pmin = float(pline[pline.index('pmin')+5:pline.index(', pmax')])
        pmax = float(pline[pline.index('pmax')+5:pline.index(')')])
        #prior_dist = sps.norm(loc=mu, scale=sigma)
        x = np.linspace(pmin, pmax, 300)
        prior = parameter.TruncNormalPrior
        y = prior(x, mu, sigma, pmin, pmax)
        #y = prior_dist.pdf(x)
    elif prior_type == 'LinearExp':
        pmin = float(pline[pline.index('pmin')+5:pline.index(', pmax')])
        pmax = float(pline[pline.index('pmax')+5:-1])
        x = np.linspace(pmin, pmax, 300)
        prior = parameter.LinearExpPrior
        y = prior(x, pmin, pmax)
...
def gorilla_bf(array, max=-4, min=-10, nbins=None, weights=None):
    """
    Function to determine if the smallest amplitude bin is more or less probable
    than the prior.
    
    Rewrite to account for possibility of including weights
    """
    if weights is None:
...
def get_bayes_fac(c, amp_param='log10_mc', weights=None):
    ...
    if np.isnan(BF) or weights is not None:
        # use gorilla BF if weights supplied
        BF = gorilla_bf(c.chain[c.burn:, idx], max=pmax, min=pmin, nbins=20, weights=weights)
    return BF
...
def cs_plot_and_set_burn(cs, param='lnpost', source_name='', burn=8000,
                         adapt_burnin=True, min_burn=2000):
    total_samples = 0
    ...
    for i in range(nchains):
        if param == 'lnpost':
            if adapt_burnin:
                ...
                burn = np.max([min_burn,idx_bad+2000])
            cs[i].set_burn(burn)
    ...
    print(total_postburn_samples,f'samples after burn in')
...
def concatenate_chains_to_single_core(cs, source_name, thin_by, chaindir,
    ...
    if upper_limit or detection:
        ...
        if vary_fgw:
            log10_mc_mu_scaled = log10_mc_mu + np.log10(u.Msun.to(u.kg)*const.G/const.c**3)
            log10_mc_mu_scaled = (5/3*log10_mc_mu_scaled - log10_dL_scaled + np.log10(2*np.pi**(2/3)))
            log10_mc_sigma_scaled = 5/3*log10_mc_sigma
            log10_h0_prior_str = ('log10_h0:Convolve(Normal(mu='
                                  f'{log10_mc_mu_scaled}, '
                                  f'sigma={log10_mc_sigma_scaled}), '
                                  'Uniform(pmin='
                                  f'{log10_fgw_prior_scaled[0]}, '
                                  f'pmax={log10_fgw_prior_scaled[1]}))')
        else:
            ...
            # I figured out it's just this factor
            log10_h0_sigma = 5/3*log10_mc_sigma
    ...
    mc_prior_str = (f'mc:Uniform(pmin={10**log10_mc_prior[0]}, '+
                          f'pmax={10**log10_mc_prior[1]})')
    c.priors = np.concatenate([c.priors, [mc_prior_str]])
...
def truncate_floats_in_priors(priors):
    def truncate_float(match):
        float_number = float(match.group())
        return f"{float_number:.3f}"
    # Regular expression to find floating-point numbers
    float_pattern = re.compile(r"-?\d+\.\d+")
    # Iterate over each string and truncate the floats
    truncated_strings = [float_pattern.sub(truncate_float, p) for p in priors]
...
def corner_single(target_str, c, dataset, project_path, save_loc=None,
                  ...
                  detection=False, upper_limit=False, weights=None, mask=None, nogwb=False, pub_fig=False,
                  target_fancy_name=None, truths={}, plot_mc_prior_mean=True,
                  Npoints=1000, pars_select=None, labels=None, titles=None, extra_str='',
                  params_no_plot_full_prior=['mc','crn_log10_A','crn_gamma','gwb_log10_A','gwb_gamma'],
                  include_lnpost=False, mc_scaled=False):
    '''
    Parameters
    ----------
    target_str : str
    ...
    weights : weights for corner (e.g. from posterior reweighting)
    mask : mask for samples (e.g. from posterior reweighting)
    ...
    params_no_plot_prior : list of strings
        Which params to not plot the whole prior range
    ...
    priors = truncate_floats_in_priors(np.copy(c.priors))
    if mask is None:
        mask = np.arange(len(c.chain[c.burn:,0]))
    ...
    if detection:
        ...
        BF = get_bayes_fac(c, weights=weights)
        print(f'{source_name} SD BF = {BF:0.3f}')
    elif upper_limit:
        save_str = target_str + '_UL'
        UL = utils.weighted_quantile(c('mc'), 0.95, sample_weight=weights)
        if mc_scaled:
            print(f'{source_name} Mc UL = {UL:0.3f}e9')
        else:
            print(f'{source_name} Mc UL = {UL/1e9:0.3f}e9')
    else:
    ...
    if pub_fig:
        ...
        for line in c.runtime_info:
            ...
            if 'gwb_log10_A' in line and not vary_crn and not nogwb:
                gwb_log10_A = np.round(float(line[line.index('=')+1:]), decimals=2)
            if 'gwb_gamma' in line and not vary_crn and not nogwb:
                gwb_gamma = np.round(float(line[line.index('=')+1:]), decimals=2)
        ...
        # Make title, start with target, dataset, luminosity distance
        suptitle = (f'{source_name} w/ enterprise + PTMCMCSampler \n'
                    f'Dataset: {dataset} \n'
                    r'$\log_{10}(d_{\rm{L}}/\rm{Mpc}) =$' + f'{log10_dL:0.3f} \n')
        # Add sky location info
        if all_sky:
            prior = [p for p in priors if 'cos_gwtheta' in p][0]
            prior = prior.replace('cos_gwtheta:', r'$\cos\theta$: ')
            suptitle += f'{prior} \n'
            prior = [p for p in priors if 'gwphi' in p][0]
        ...
        # Add frequency prior
        if vary_fgw:
            prior = [p for p in priors if 'log10_fgw' in p][0]
            prior_type = prior[prior.index(':')+1:prior.index('(')]
            prior = prior.replace('log10_fgw:', r'$\log_{10}f_{\rm{GW}}$: ')
            suptitle += f'{prior} \n'
        else:
            suptitle += r'$\log_{10}(f_{\rm{GW}}/\rm{Hz}) =$' + f'{log10_fgw} \n'
        # Add chirp mass prior info
        prior = [p for p in priors if 'log10_mc' in p][0]
        prior = prior.replace('log10_mc:', r'$\log_{10}\mathcal{M}_c$: ')
        suptitle += f'{prior} \n'
        # Add CRN info
        if vary_crn:
            prior = [p for p in priors if 'crn_log10_A' in p or
                     'gwb_log10_A' in p][0]
            prior = prior.replace('crn_log10_A:', r'$\log_{10}A_{\rm{CRN}}$: ')
            prior = prior.replace('gwb_log10_A:', r'$\log_{10}A_{\rm{GWB}}$: ')
            suptitle += f'{prior} \n'
            prior = [p for p in priors if 'crn_gamma' in p or
                     'gwb_gamma' in p][0]
            ...
            try:
                ...
                suptitle += (r'$\log_{10}A_{\rm{GWB}} =$' + f'{gwb_log10_A} \n'
                             r'$\gamma_{\rm{GWB}} =$' + f'{gwb_gamma} \n')
        # params not shown
        if np.any(['p_dist' in p for p in c.params]) and np.any(['red_noise' in p for p in c.params]):
            Npsr = len([p for p in c.params if 'p_dist' in p])
            suptitle += f'{4*Npsr} '+r'PSR params not shown: RN, $L_p$, $\Phi_p$' + ' \n'
        elif np.any(['p_dist' in p for p in c.params]):
            Npsr = len([p for p in c.params if 'p_dist' in p])
            suptitle += f'{2*Npsr} '+r'PSR params not shown: $L_p$, $\Phi_p$' + ' \n'
        elif np.any(['red_noise' in p for p in c.params]):
            Npsr = len([p for p in c.params if 'red_noise_gamma' in p])
            suptitle += f'{2*Npsr} PSR params not shown: RN \n'
        N_custom = len([p for p in c.params if np.any([n in p for n in ['dm_gp','chrom_gp','exp','sw_r2']])])
        N_custom_psrs = len(np.unique([p.split('_')[0] for p in c.params if
                                       np.any([n in p for n in ['dm_gp','chrom_gp','exp','sw_r2']])]))
        if N_custom > 0:
            suptitle += f'{N_custom} custom noise params not shown for {N_custom_psrs} PSRs \n'
        # Add sampler info
        suptitle += (f'{nsamples} samples \n' +
                     r'Highest $\hat{R}-1$ statistic: ' + f'{np.round(max_Rhat-1,decimals=3)} \n')
        # report BF or upper limit
        if detection:
            suptitle += 'SD BF: $\mathcal{B} = $'+f'{BF:0.3f} \n'
        elif upper_limit:
            if mc_scaled:
                suptitle += (r'$\mathcal{M}_c$ UL ='+f'{UL:0.3f}'+r'$\cdot 10^9$ M$_\odot$'+' \n')
            else:
                suptitle += (r'$\mathcal{M}_c$ UL ='+f'{UL/1e9:0.3f}'+r'$\cdot 10^9$ M$_\odot$'+' \n')
        else:
    ...
    if pars_select is None and labels is None and titles is None:
        ...
        titles = [r'$\cos\iota$', '$\Phi_{0,\mathrm{GW}}}$', r'$\psi$', r'$\log_{10}h_0$',
                  r'$\log_{10}\mathcal{M}_c$']
        if mc_scaled:
            labels = [r'$\cos\iota$', '$\Phi_{0,\mathrm{GW}}}$', r'$\psi$', r'$\log_{10}h_0$',
                      r'$\log_{10}(\mathcal{M}_c/10^9$ M$_\odot)$']
        else:
            labels = [r'$\cos\iota$', '$\Phi_{0,\mathrm{GW}}}$', r'$\psi$', r'$\log_{10}h_0$',
        ...
        if include_lnpost:
            pars_select += ['lnpost']
            titles += [r'$\log(\mathcal{L}\pi)$']
            labels += [r'$\log(\mathcal{L}\pi)$']
    else:
        save_str += '_custom_params'

    # plot
    idxs = [c.params.index(p) for p in pars_select]
    fig = corner.corner(c.chain[c.burn:, idxs][mask], labels=labels,
                        ...
                        title_kwargs={'fontsize': 14}, weights=weights)
    if not pub_fig:
    ...
    for i, p in enumerate(pars_select):
        ...
        if not p in params_no_plot_full_prior:
    ...
    for yi, p1 in enumerate(pars_select): # rows
        for xi, p2 in enumerate(pars_select[:yi]): # cols
            ax = axes[yi, xi]
            if not p2 in params_no_plot_full_prior and not p2 == 'lnpost':
                x, _ = get_prior_distr(c, p2)
                ax.set_xlim([x.min(),x.max()])
            if not p1 in params_no_plot_full_prior and not p1 == 'lnpost':
    ...
    if save_loc:
        if pub_fig:
            fig.savefig(f'{save_loc}/{save_str}{extra_str}.pdf',
                        format='pdf', dpi=600)
            fig.savefig(f'{save_loc}/{save_str}{extra_str}.png', dpi=600)
        else:
            fig.savefig(f'{save_loc}/{save_str}{extra_str}.png')
    plt.show()
...
def free_spec(cs, psrs, gw_basename='crn', save_loc=None, colors=None,
              ...
              pmin=-10, title=None, weights=None):

...
def get_parameter_groups_CAW_target(pta):
    ...
    chrom_signals = ['dm_gp', 'chrom_gp', 'sw_r2', 'sw_gp', 'exp_1', 'exp_2', 'exp1', 'exp', 'exp2']

    for sc in pta._signalcollections:
        for signal in sc._signals:
            # note 'red noise' name may also apply to a GWB
            if signal.signal_name in ['red noise']:
                ...
                wnpars.extend(signal.param_names)
            elif signal.signal_name in chrom_signals:
    ...
    if 'red noise' in snames:
        ...
        rnpsrs = np.unique([ p.split('_')[0] for p in params if 'red_noise_log10_A' in p])
        ...
        for psr in rnpsrs:
            ...
            if 'gwb_log10_A' in params and 'gwb_gamma' in params and 'gwb_log10_fb' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'),
                                params.index('gwb_gamma'), params.index('gwb_log10_A'), params.index('gwb_log10_fb')]])
            elif 'gwb_log10_A' in params and 'gwb_gamma' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'),
                                params.index('gwb_gamma'), params.index('gwb_log10_A')]])
            elif 'gwb_log10_A' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'), params.index('gwb_log10_A')]])
            if 'crn_log10_A' in params and 'crn_gamma' in params and 'crn_log10_fb' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'),
                                params.index('crn_gamma'), params.index('crn_log10_A'), params.index('crn_log10_fb')]])
            elif 'crn_log10_A' in params and 'crn_gamma' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'),
                                params.index('crn_gamma'), params.index('crn_log10_A')]])
            elif 'crn_log10_A' in params:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A'), params.index('crn_log10_A')]])
            if len(gwfspars) > 0:
                groups.extend([[params.index(psr + '_red_noise_gamma'),
                                params.index(psr + '_red_noise_log10_A')] + [params.index(p) for p in gwfspars]])
            if psr + '_hf_red_noise_log10_A' in params and psr + '_hf_red_noise_gamma' in params:
                groups.extend([[params.index(psr + '_hf_red_noise_gamma'),
                                params.index(psr + '_hf_red_noise_log10_A')]])
                if psr + '_red_noise_log10_A' in params and psr + '_red_noise_gamma' in params:
                    groups.extend([[params.index(psr + '_hf_red_noise_gamma'),
                                    params.index(psr + '_hf_red_noise_log10_A'),
        ...
        if 'gwb_log10_A' in params:
            ...
            if 'gwb_log10_fb' in params:
                b.append(params.index('gwb_log10_fb'))
            a.extend(b)
            groups.extend([a])
            groups.extend([b])
            groups.extend([b])
            groups.extend([b])
        if 'crn_log10_A' in params:
            b.append(params.index('crn_log10_A'))
            if 'crn_gamma' in params:
                b.append(params.index('crn_gamma'))
            if 'crn_log10_fbend' in params:
                b.append(params.index('crn_log10_fbend'))
            if 'crn_log10_fb' in params:
                b.append(params.index('crn_log10_fb'))
            a.extend(b)
            groups.extend([a])
            groups.extend([b])
            groups.extend([b])
    ...
    for i, chrom_signal in enumerate(chrom_signals):
        if chrom_signal in snames:
            ...
            for psr in chrompsrs:
                ...
                # cross chrom and red noise groups
                ...
                # cross chromatic groups
                for chrom_signal2 in chrom_signals[i+1:]:
                    if f'{psr}_{chrom_signal2}' in pta.signals:
                        chrom2_params = pta.signals[f'{psr}_{chrom_signal2}'].param_names
                        groups.extend([[params.index(par) for par in chrom_params + chrom2_params]])
        
    # all chrom params
    groups.extend([[params.index(p) for p in chrompars]])
        
    # make group with all chromatic params + red noise for each pulsar
    # redundant - this is already done above
    ...
    if 'cw' in snames:
        ...
        if len(cw_pulsar) > 0:
            ...
            dropout_params = [ p for p in cw_common if 'k_drop' in p ]
            
            for pd,pp in zip(pdist_params,pphase_params):
                pname = pd.split('_')[0]
                for dp in dropout_params:
                    if pname in dp:
                        if 'cos_gwtheta' in params and 'gwphi' in params:
                            groups.extend([[params.index(dp), params.index(pd),
                                            params.index('cos_gwtheta'), params.index('gwphi')]])
                        groups.extend([[params.index(dp), params.index(pd), params.index('log10_mc')]])
                        groups.extend([[params.index(dp), params.index(pp),
                                        params.index('phase0'), params.index('log10_mc')]])
                        groups.extend([[params.index(dp), params.index(pp), params.index('phase0'),
                                        params.index('log10_mc'), params.index('cos_inc'), params.index('psi')]])
                        groups.extend([[params.index(dp), params.index(pd), params.index(pp), 
                                        params.index('log10_mc')]])
                        if 'log10_fgw' in cw_common:
                            groups.extend([[params.index(dp), params.index(pd), params.index('log10_fgw')]])
                            groups.extend([[params.index(dp), params.index(pp),
                                            params.index('phase0'), params.index('log10_fgw')]])
                            groups.extend([[params.index(dp), params.index(pp), params.index('phase0'),
                                            params.index('log10_fgw'), params.index('cos_inc'), params.index('psi')]])
                            groups.extend([[params.index(dp), params.index(pd), params.index(pp), 
                                            params.index('log10_fgw')]])
                            groups.extend([[params.index(dp), params.index(pd), params.index(pp), 
                                            params.index('log10_fgw'), params.index('log10_mc')]])
                            groups.extend([[params.index(dp), params.index(pp), params.index('phase0'),
                                            params.index('log10_mc'), params.index('log10_fgw')]])
                            groups.extend([[params.index(dp), params.index(pp), params.index('phase0'),
                                            params.index('log10_mc'), params.index('cos_inc'),
                                            params.index('psi'), params.index('log10_fgw')]])
                    else:
                        if 'cos_gwtheta' in params and 'gwphi' in params:
                        ...
                        if 'log10_fgw' in cw_common:
                            ...
                            groups.extend([[params.index(pd), params.index(pp), 
                                            params.index('log10_fgw'), params.index('log10_mc')]])
                            groups.extend([[params.index(pp), params.index('phase0'),
                                            params.index('log10_mc'), params.index('log10_fgw')]])
                            groups.extend([[params.index(pp), params.index('phase0'), params.index('log10_mc'), 
        ...
        bpl_pars = ['gwb_gamma', 'gwb_log10_A', 'gwb_log10_fbend', 'gwb_log10_fb',
                    'crn_gamma', 'crn_log10_A', 'crn_log10_fbend', 'crn_log10_fb']

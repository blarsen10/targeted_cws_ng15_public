...
if os.path.isfile(f'{outdir}/pta.pkl'):
    ...
    if args.vary_crn:
        ...
        print(f'Using fixed CRN params.')
...
for chain_idx, new_array_idx in zip(chain_idxs, new_array_idxs):
    ...
    if not crn_params == []:
        if 'crn_log10_rho' in crn_params[0]:
    ...
    ln_weights[new_array_idx] = hd_pta.get_lnlikelihood(params) - core_crn('lnlike')[chain_idx]
